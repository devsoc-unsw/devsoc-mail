...
| GET       | /mail/view          | header `session`                       | `{ "mails": [ { "mailId": 1234, "sender": "eve@devsoc.mail", "receivers": ["a@devsoc.mail", "b@devsoc.mail"], "title": "amazing subject", "message": "hola", "timeSent": 1683019484, "readBy": ["a@devsoc.mail"] } ] }` | Returns a list of all the user’s existing mail                  | 401 Invalid session                                                                                        |
| GET       | /mail/:mailId       | path `mailId`, header `session`         | `{ "mail": { "mailId": 1234, "sender": "eve@devsoc.mail", "receivers": ["a@devsoc.mail", "b@devsoc.mail"], "title": "amazing subject", "message": "hola", "timeSent": 1683019484, "readBy": ["a@devsoc.mail"] } }` | View one specific mail in a user’s inbox                        | 400 Mail ID does not exist in user’s inbox<br>401 Invalid session                                         |
...
| PUT       | /mail/read          | header `session`, body `mailId`        | `{}`                                                             | Marks mail as read if unread              | 401 Invalid session<br>400 mail ID does not exist                                                           |
